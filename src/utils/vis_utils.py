...
from src.utils.geometry import compute_alignment_transforms, pose_difference, rot2quat
from src.graph.data_classes import FrontierType

FT_GAIN_MAX = 20.0

# BGR colors per frontier type (used in RGB overlay arrows)
_FRONTIER_TYPE_COLOR_BGR = {
    FrontierType.FLOOR:     (255, 130,  70),  # blue
    FrontierType.DOOR:      (255,   0, 200),  # purple
    FrontierType.FURNITURE: (  0, 220, 255),  # yellow
}
...
def visualize_2D_frontier(
    ...
    for ft in ft_3D:
        col = _FRONTIER_TYPE_COLOR_BGR.get(ft.frontier_type, (200, 200, 200))
...
# Frontier debug overlays

def draw_frontier_debug(
    obs_rgb: np.ndarray,
    frontiers: List[Any],
    img_size: Tuple[int, int],
) -> np.ndarray:
    """
    Draw frontier overlays on an RGB image with ID and depth annotations.

    Calls ``visualize_2D_frontier`` for the coloured arrows, then adds a
    text label (``#<id> d=<depth>m``) next to each frontier centroid.

    Parameters
    ----------
    obs_rgb : (H, W, 3) uint8 RGB image
    frontiers : list of Frontier objects
    img_size : (H, W) image dimensions

    Returns
    -------
    vis : (H, W, 3) uint8 annotated image (copy of obs_rgb)
    """
    vis = visualize_2D_frontier(frontiers, obs_rgb, img_size)
    ...
    for ft in frontiers:
        ...
        label = f"#{ft.id} d={ft.depth_estimate:.1f}m s={ft.score:+.2f}"
        cv2.putText(
            vis, label,
            (px + 8, py - 8),
...
def draw_candidate_mask_debug(
    depth: np.ndarray,
    detector,
) -> np.ndarray:
    """
    Visualize frontier candidate pixels overlaid on a jet-coloured depth map.

    Runs the detector's internal candidate-mask computation (Sobel
    discontinuities + missing-depth boundaries) and draws the result
    in green on top of the depth image.

    Parameters
    ----------
    depth : (H, W) float depth image (metres)
    detector : FrontierDetectorDepth instance

    Returns
    -------
    vis : (H, W, 3) uint8 BGR image suitable for cv2.imwrite
    """
    from src.utils.geometry import compute_valid_mask

    cfg = detector.cfg
    depth_f32 = depth.astype(np.float32)

    valid_mask = compute_valid_mask(
        depth_f32, cfg.near_depth, cfg.far_depth, cfg.top_crop_fraction,
    )
    depth_clean = np.where(valid_mask, depth_f32, 0.0)
    candidate_mask = detector._compute_candidate_mask(depth_clean, valid_mask)

    # Normalize depth to 0-255 for visualization
    d_vis = depth_f32.copy()
    d_vis[~valid_mask] = 0.0
    d_max = d_vis.max() if d_vis.max() > 0 else 1.0
    d_norm = (d_vis / d_max * 255).astype(np.uint8)
    d_rgb = cv2.applyColorMap(d_norm, cv2.COLORMAP_JET)

    # Overlay candidate mask in green
    d_rgb[candidate_mask > 0] = [0, 255, 0]
